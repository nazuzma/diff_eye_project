...
def read_video_and_get_all_derivative_and_save():
    ...
    read_frames_time_in_ms = 100  # time in miliseconds at which u want to read video, it will be 100,200,300,400....
    ...
    out = cv2.VideoWriter(root + '\\output_videos\\outputWithTime.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'),
                          new_fps,
                          (width, height), 0)
    ...
    while True:  # if statement id true, execute these set of statements
        video.set(cv2.CAP_PROP_POS_MSEC,
                  int(
                      read_frames_time_in_ms * current_frame_number))  # we are telling opencv that we want to read the frame at this specifc time
